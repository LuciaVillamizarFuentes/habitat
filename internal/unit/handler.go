...
// Handler expone los casos de uso de unidades por HTTP.
type Handler struct {
	svc *Service
}
...
func (h *Handler) Routes(mux *http.ServeMux) {
	mux.HandleFunc("POST /api/v1/units", h.create)
	mux.HandleFunc("GET /api/v1/units", h.list)
	mux.HandleFunc("GET /api/v1/units/{id}", h.get)
}
...
func writeErr(w http.ResponseWriter, err error) {
	...
	case errors.Is(err, ErrInvalidCode), errors.Is(err, ErrInvalidKind):
		httpx.Error(w, http.StatusUnprocessableEntity, err.Error())
	case errors.Is(err, ErrDuplicateCode):
		httpx.Error(w, http.StatusConflict, err.Error())
	case errors.Is(err, ErrInvalidFloor), errors.Is(err, ErrInvalidArea), errors.Is(err, ErrInvalidCoefficient):
		httpx.Error(w, http.StatusUnprocessableEntity, err.Error())
	default:
